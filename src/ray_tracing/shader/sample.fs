...
uniform element elements[4];
uniform atom atoms[24];
...
vec2 rand2Uniform() {
    vec2 r2 = texture(tRand2Uniform, gl_FragCoord.xy/randsize + rand.xy + randState).rg;
    randState += r2;
    return r2;
}

vec2 rand2Normal() {
    vec2 r2 = texture(tRand2Normal, gl_FragCoord.xy/randsize + rand.xy + randState).rg;
    randState += r2;
    return r2;
}

vec3 rand3Normal() {
    vec3 r3 = texture(tRand3Normal, gl_FragCoord.xy/randsize + rand.xy + randState).rgb;
    randState += r3.xy;
    return r3;
}
...
bool intersect(vec3 r0, vec3 rd, out vec3 pos, out vec3 norm, out vec3 color, out float roughness, out bool light) {
    ...
    for (int i = 0; i < 24; i++) {
        ...
        if (raySphereIntersect(r0, rd, s, elements[atoms[i].element].radius * 1.2, t)) {
            if (t < tmin) {
                tmin = t;
                pos = r0 + rd * t;
                norm = normalize(pos - s);
                roughness = atom_roughness;
                color = elements[atoms[i].element].color;
                light = false;
                hit = true;
            }
        }
...
void main() {
    // buildMolecule();
    vec3 lightPos = vec3(cos(light_angle) * 8.0, 8., sin(light_angle) * 8.0);
    vec3 src = texture(source, gl_FragCoord.xy/resolution).rgb;
    vec2 jitter = vec2(0);
    if (antialias) {
